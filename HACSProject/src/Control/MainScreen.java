 ...
 *
 * This is a GUI class. Most of it is autogenerated by netbeans. It has a
 * ComboBox of existing RFID's that feeds the selected RFID into a TextBox. When
 * TextBox has an RFID which exists in the database the user can get and update
 * the configuration belonging to that RFID. This class does nothing other than
 * feeding into and taking out of the database..
 *
 * auth@ Eiríkur Emilsson and Gunnar Ómarsson. 
 */
package Control;
...
public class MainScreen extends javax.swing.JFrame {
    ...
    /**
     * @brief firstRun is just to keep track of whether or not the combobox is being
...
//Búa til clear á þetta og method til að geta kallað þennan lista fram aftur og aftur. 
    ...
    private void initComponents() {
        ...
        validRFIDCheckBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                validRFIDCheckBoxActionPerformed(evt);
            }
        });
     ...
     *
     * This button creates a line for the user in the database. The user/RFID is
     * created with a 1234 pin number and empty configuration otherwise. If an
     * RIFD already exists it is not updated and the configuration is unchanged.
     */
    private void createUserButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_createUserButtonActionPerformed
        ...
        try {
            ...
            TextPanel.setText("RFID " + RFID1 + " has been created in database.");
        } catch (SQLException ex) {
            TextPanel.setText("ERROR!!!!  Please write an valid RFID and make sure it doesn't already exist. Pin, validity and configuration items are set later.");
        }
     ...
     *
     * Once a configuration for a given RFID is changed it can be saved with
     * this button. It is very dangerous in the sense that it is very easy to
     * overwrite a configuration with either an empty configuration or a
     * configuration for a different RFID and there is no prompt to save the
     * user.
     */
    private void updateConfigButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateConfigButtonActionPerformed
        ...
        try {
            ...
            TextPanel.setText("Configuration for  " + RFID1 + " has been created in database.");
        } catch (NumberFormatException ex) {
            TextPanel.setText("ERROR!!!! Please use numbers only for each configuration item. Pin should be a 4 letter number and each configuration item a number between 0 and 100.");
        } catch (SQLException ex) {
            TextPanel.setText("ERROR!!! Check the database is ok and that you have a valid RFID");

        }

    }//GEN-LAST:event_updateConfigButtonActionPerformed
    /**
     * @brief Item field for configuration for a single item in the
     * configuration.
     */
     ...
     *
     * This field is for the RFID which the user wants to work with. The user
     * should be able to write into it or get it from the combobox. All the
     * buttons in the RFID: Delete and Create User + get and update
     * configuration, all refer to the content in this field.
     */
     ...
     *
     * This button takes the RFID in the RFIDTextField and deletes its entry in
     * the database. The GUI survives any wrong or illegal entry from the
     * RFIDTextField that we have tested. The method then calls for a refresh of
     * the combobox to reflect the changes in the database.
     */
    private void delUserButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_delUserButtonActionPerformed
        ...
        try {
            ...
            refreshRFIDList();
            TextPanel.setText("New User " + RFID1 + " has been deleted from database.");
        } catch (SQLException ex) {
            TextPanel.setText("ERROR !!!! RFID " + RFID1 + " can not be found in database.");
        }


        //RFIDList.removeAllItems();  //Delete everything in the RFID comboBox
        /*
         * try { Database.QueryExchange delUserObj = new
         * Database.QueryExchange(); delUserObj.delUser(RFID1); } catch
         * (SQLException ex) { Logger.getLogger(RFID1); //RFID1 as payload makes
         * no sense. TODO. }
         */
     ...
     *
     * This method takes the RFID in the RFIDTextField and gets the
     * configuration for it and populates into the relevant fields. Nothing seen
     * so far crashes the GUI when running this method.
     */
    private void getConfigButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_getConfigButtonActionPerformed
        ...
        try {
            ...
            TextPanel.setText("Configuration for  " + RFID1 + " fetched.");
        } catch (SQLException ex) {
            TextPanel.setText("Configuration for  " + RFID1 + " can not be found in database.");
        }
    ...
    /**
     * @brief BBuilds the RFID combobox.
     *
     * Gets the entries for the RFID's in database and populates the combobox
     * with them. The main text should be "Select or write RFID" if the method
    ...
    private void validRFIDCheckBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_validRFIDCheckBoxActionPerformed
           
    }//GEN-LAST:event_validRFIDCheckBoxActionPerformed
    /**
     * @brief Refreshes the combobox.
     *
     * This method is called when the combobox needs to be updated. The combobox
     * has all the RFID's that are in the database but the list is compiled at
     * runtime of the Mainscreen class. Changes to the list are not
     * automatically updated and so must be refreshed with a method like this.
     * Changes that are updated with this method are when a user is created or
     * deleted.
     */
