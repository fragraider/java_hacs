 ...
 * and 
 * @version 29/09/10
 * @author ibr, hbe  -  Eirikur Emilsson wrote the checkSum method. 
 */
public final class HacsPacket extends Packet {
    ...
    /**
     * @Brief Generates the checksum value.
     * 
     * Generates the checkSum by XORing each character in data against the next. Finally the String is built using the checkSum and a parameter to change it over to decimal. 
     */
    ...
    public String makeCheckSum(String data){
        ...
        tempCheckSum = (tempCheckSum ^ data.charAt(checkChar));
        checkChar++;
        }
        
        String checkSum = String.format("%03X",tempCheckSum);  //
        return checkSum;
        }
